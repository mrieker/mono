...
sgen_register_root (char *start, size_t size, SgenDescriptor descr, int root_type, int source, const char *msg)
{
	...
	LOCK_GC;
	rc = sgen_register_root_locked (start, size, descr, root_type, source, msg);
	UNLOCK_GC;
	return rc;
}

int
sgen_register_root_locked (char *start, size_t size, SgenDescriptor descr, int root_type, int source, const char *msg)
{
	RootRecord new_root;
	int i;
